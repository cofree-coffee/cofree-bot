...
module CofreeBot.Bot.GHCI.Interpreter
  ( runGhci
  ) where
...
interleave as = filter (not . null . fst) . zipping
  isSilent
  (\a -> if isReallySilent a then ("", Nothing) else (a, Nothing))
...
isSilent str | isPrefixOf ":set " str     = True
...
isReallySilent str | isPrefixOf "@" str = True
                   | otherwise          = False
...
readProcess' cmd args =
  readCreateProcess $ shell $ intercalate " " (cmd : args) <> " 2>&1"
