...
parseUsername =
  Username <$> Opt.strOption
      (Opt.long "username"
    <> Opt.metavar "STRING"
    <> Opt.help  "The fully qualified user ID or just local part of the user ID, to log in.")

parsePassword :: Opt.Parser LoginSecret
parsePassword =
  Password <$> Opt.strOption
      (Opt.long "password"
    <> Opt.metavar "STRING"
    <> Opt.help "The user's password.")

parseDeviceId :: Opt.Parser (Maybe DeviceId)
parseDeviceId = Opt.optional $
  DeviceId <$> Opt.strOption
      (Opt.long "device_id"
    <> Opt.metavar "STRING"
    <> Opt.help "ID of the client device. If this does not correspond to a known client device, a new device will be created. The server will auto-generate a device_id if this is not specified.")

parseInitialDeviceName :: Opt.Parser (Maybe InitialDeviceDisplayName)
parseInitialDeviceName = Opt.optional $
  InitialDeviceDisplayName <$> Opt.strOption
      (Opt.long "initial_device_name"
    <> Opt.metavar "STRING"
    <> Opt.help "A display name to assign to the newly-created device. Ignored if device_id corresponds to a known device.")
